function [ phi ] = G1_ChanVeseIpol_GDExp( I, phi_0, mu, nu, eta, lambda1, lambda2, tol, epHeaviside, dt, iterMax, reIni )
...
%     c2 = sum(I(2:end-1,2:end-1).*(1-H(2:end-1,2:end-1))) / sum(1-H(2:end-1,2:end-1)); %TODO 2: Line to complete
    ...
    c1 = sum(sum(I(phi>=0))) / sum(sum((phi>=0))); %TODO 1: Line to complete
    c2 = sum(sum(I(phi<0))) / sum(sum((phi<0))); %TODO 2: Line to complete
    ...
    delta_phi = G1_diracReg(phi_old, epHeaviside);   %notice delta_phi=H'(phi)	
